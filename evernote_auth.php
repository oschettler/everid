<?php
    ...
    define("EVERNOTE_LIBS", dirname(__FILE__) . DIRECTORY_SEPARATOR . "vendor/evernote/evernote/lib");
    ...
    if (!class_exists('OAuth')) {
      flash('error', 'The PHP OAuth Extension is not installed');
      die();
    }

    // Verify that you have configured your API key
    if (strlen(config('oauth.consumer_key')) == 0 
      || strlen(config('oauth.consumer_secret')) == 0) {
      flash('error', 'Please set oauth.consumer_key and oauth.consumer_secret in your settings.ini');
      die();
    }
    ...
    function getTemporaryCredentials()
    {
        ...
        try {
            ...
            if ($requestTokenInfo) {
                ...
                flash('Failed to obtain temporary credentials.');
            }
        } catch (OAuthException $e) {
            flash('Error obtaining temporary credentials: ' . $e->getMessage());
        }
    ...
    function handleCallback()
    {
        ...
        if (isset($_GET['oauth_verifier'])) {
            ...
            flash('Content owner did not authorize the temporary credentials');
    ...
    function getTokenCredentials()
    {
        ...
        if (isset($_SESSION['accessToken'])) {
            flash('Temporary credentials may only be exchanged for token credentials once');
        ...
        try {
            ...
            if ($accessTokenInfo) {
                ...
                flash('Failed to obtain token credentials.');
            }
        } catch (OAuthException $e) {
            flash('Error obtaining token credentials: ' . $e->getMessage());
        }
    ...
    function listNotebooks()
    {
        ...
        try {
            ...
            if (isset(EDAMErrorCode::$__names[$e->errorCode])) {
                flash('Error listing notebooks: ' . EDAMErrorCode::$__names[$e->errorCode] . ": " . $e->parameter);
            } else {
                flash('Error listing notebooks: ' . $e->getCode() . ": " . $e->getMessage());
            }
            ...
            flash('Error listing notebooks: ' . $e->getMessage());
        }
